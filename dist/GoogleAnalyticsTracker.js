...
class GoogleAnalyticsTracker {
     ...
     * @param  {HitPayload} payload (Optional) An object containing the hit payload
     */
    trackEvent(category, action, eventMetadata, payload = null) {
        this.transformPayload(payload);
        AnalyticsBridge.trackEvent(this.id, category, action, (eventMetadata && eventMetadata.label ? eventMetadata.label : null), (eventMetadata && eventMetadata.value != null ? eventMetadata.value.toString() : null), payload);
    }
    /**
     ...
     * @param  {HitPayload} payload (Optional) An object containing the hit payload
     */
    trackTiming(category, interval, timingMetadata, payload = null) {
        this.transformPayload(payload);
        AnalyticsBridge.trackTiming(this.id, category, interval, timingMetadata.name, timingMetadata.label, payload);
    }
    /**
     * Track an exception
     * @example
    ...
    dispatchWithTimeout(timeout = -1) {
        ...
        let timer = null;
        const withTimeout = (timeout) => new Promise(resolve => {
            timer = setTimeout(() => {
                timer = null;
                resolve(false);
